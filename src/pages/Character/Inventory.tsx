...
export function useInventoryPanels({ inventory, vendorMode, defaultCapacity, onMoneyChange, onInventoryChange, onDropItem, onSellItem, onUseItem, onEquipItem }: { inventory?: Character.Item.TInventory; vendorMode?: boolean; defaultCapacity?: number; onMoneyChange?: (money: Character.Item.TMoney) => Promise<void> | void; onInventoryChange?: (inventory: Character.Item.TInventory) => Promise<void> | void; onDropItem?: (source: ServerApi.CharacterRoutes.ItemActionSource) => Promise<void> | void; onSellItem?: (source: ServerApi.CharacterRoutes.ItemActionSource, requestedPriceCopper: number) => Promise<void> | void; onUseItem?: (source: ServerApi.CharacterRoutes.ItemActionSource) => Promise<void> | void; onEquipItem?: (source: ServerApi.CharacterRoutes.ItemActionSource, targetSlotId?: string, target?: ServerApi.CharacterRoutes.ItemActionSource) => Promise<void> | void; }) {
  ...
  const equipmentPanel = <FlexRow className="w-full min-w-0 shrink-0 justify-center items-start"><div className="relative w-full border p-1 fancy-container bg-center bg-contain bg-no-repeat" style={{ aspectRatio: "670 / 887", backgroundImage: "url('/imgs/character_3.svg')" }}>{equippedItems.map((cell, index) => <div key={`eq-${index}`} className={`aspect-square absolute border text-center select-none origin-top-left ${cell ? "border-slate-950 bg-slate-500/45 shadow-[0_0_0_1px_rgba(15,23,42,0.7)]" : "border-slate-500/70 bg-slate-800/35"} fancy-container ${absPositions[index][0] === "bag" || absPositions[index][0] === "satchel" ? "w-[13%]" : ""}`} style={{ top: absPositions[index][1], left: absPositions[index][2], width: absPositions[index][0] === "bag" || absPositions[index][0] === "satchel" ? "13%" : "18%" }} onDragOver={handleDragOver} onDrop={handleTargetDrop(index)} draggable={!!cell} onDragStart={cell ? handleDragStart(cell.item, "equipment", index) : undefined} onMouseEnter={cell ? (e) => scheduleHover(cell.item, e.clientX, e.clientY) : undefined} onMouseLeave={cell ? hideHover : undefined} onContextMenu={cell ? (e) => { e.preventDefault(); e.stopPropagation(); hideHover(); setItemMenu({ x: e.clientX, y: e.clientY, from: "equipment", index, item: cell.item }); } : undefined}>{cell ? <div className="relative flex h-full min-w-0 items-center justify-center overflow-hidden text-[9px]">{renderItemVisual(cell.item)}<span className="absolute inset-0 flex items-end justify-center overflow-hidden bg-black/25 px-0.5 pb-0.5 text-center leading-tight text-white [overflow-wrap:anywhere]"><span className="max-h-[2.4em] overflow-hidden">{cell.item.name}{cell.amount > 1 ? ` x${cell.amount}` : ""}</span></span></div> : <FlexRow className="w-full h-full items-center justify-center">{getEmptySlotIcon(absPositions[index][0])}</FlexRow>}</div>)}</div></FlexRow>;
  ...
  const moneyModal = isMoneyModalOpen ? createPortal(<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-2"><div className="fancy-container p-1 w-[min(420px,95vw)] max-h-[90vh] overflow-auto flex flex-col gap-1"><p className="font-bold text-center">Erszény</p><MoneyDisplay copper={inventoryMoneyToCopper(inventory?.money || defaultMoney)} className="justify-center" /><MoneyAddInput id="character-money-delta" valueCopper={moneyInputCopper} onChange={setMoneyInputCopper} /><FlexRow className="justify-end gap-1 flex-wrap"><button className="fancy-container p-0.5" onClick={() => setIsMoneyModalOpen(false)}>Bezárás</button><button className="fancy-container p-0.5" onClick={() => applyMoneyDelta(1)}>Hozzáadás</button><button className="fancy-container p-0.5" onClick={() => applyMoneyDelta(-1)}>Kivétel</button></FlexRow></div></div>, document.body) : null;
  ...
  const sellModal = sellTarget ? createPortal(<div className="fixed inset-0 bg-black/50 z-[100005] flex items-center justify-center p-2"><div className="fancy-container p-2 w-[min(420px,95vw)] flex flex-col gap-2"><p className="font-bold text-center">Eladas - {sellTarget.item.name}</p><MoneyAddInput id="character-sell-price" label="Kért ár" valueCopper={sellInputCopper} onChange={setSellInputCopper} /><FlexRow className="justify-end gap-1 flex-wrap"><button className="fancy-container p-0.5" onClick={() => setSellTarget(null)}>Bezárás</button><button className="fancy-container p-0.5" onClick={() => void submitSellAction()}>Küldés</button></FlexRow></div></div>, document.body) : null;
